...
import { selectNumberOfQuestions, selectNumOfWrong } from '../../../../store/questions/question-selector';
...
export class ExamScoreComponent implements AfterViewInit , OnDestroy{
  ...
  getNumOfWrong(){
    this.NumOfWrongQId = this._Store.select(selectNumOfWrong).subscribe({
      next:(num)=>{ this.numOfWrong = num }
    })
  }

  getNumOfCorrect(){
    this.NumOfCorrectQId = this._Store.select(selectNumberOfQuestions).subscribe({
      next:(num)=> { this.numOfCorrect = num - this.numOfWrong }
    })
  }
